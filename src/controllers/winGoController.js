...
import { calculatePayoutsWithColors } from "./wingoHack";
...
const addWinGo = async (game) => {
  ...
  try {
    ...
    const [allBets] = await connection.query(
      `SELECT * FROM minutes_1 WHERE status = 0 AND stage = "${period}"`
    );
    let amount;
    if (allBets.length > 0) {
      amount = calculatePayoutsWithColors(allBets)
    } else {
...
const handlingWinGo1P = async (typeid) => {
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    const [users] = await connection.execute(
      "SELECT `bonusMoney` FROM `users` WHERE `phone` = ?",
      [phone]
    );
    let totals = users[0].bonusMoney + nhan_duoc;

    await connection.execute(
      "UPDATE `minutes_1` SET `get` = ?, `status` = 1 WHERE `id` = ? ",
      [nhan_duoc, id]
    );
    const sql = "UPDATE `users` SET `bonusMoney` = ? WHERE `phone` = ? ";
