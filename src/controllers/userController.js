...
const verifyCode = async (req, res) => {
  ...
  if (user.time_otp - now <= 0) {
    request(
      `https://www.fast2sms.com/dev/bulkV2?authorization=izN2PrfjOg40tJvwWkBa59TyoDEnhVM7Zp3GCSAlm1FYebQLIxTJnRY8drPMLSuqcOVQslB4h23Zv96E&variables_values=${otp}&route=otp&numbers=${user.phone}`,
  ...
  if (data) {
    if (data == 1) {
      ...
      if (check >= data && point_list?.total1 != 0) {
        await connection.query(
          ...
          [point_list?.total1 || 2, rows[0].phone]
        );
        await connection.query(
    ...
    if (data == 2) {
      ...
      if (check >= get && point_list?.total2 != 0) {
        await connection.query(
          "UPDATE users SET money = money + ? WHERE phone = ? ",
          [point_list?.total2 || 4, rows[0].phone]
        );
        await connection.query(
    ...
    if (data == 3) {
      ...
      if (check >= get && point_list?.total3 != 0) {
        await connection.query(
          "UPDATE users SET money = money + ? WHERE phone = ? ",
          [point_list?.total3 || 8, rows[0].phone]
        );
        await connection.query(
    ...
    if (data == 4) {
      ...
      if (check >= get && point_list?.total4 != 0) {
        await connection.query(
          "UPDATE users SET money = money + ? WHERE phone = ? ",
          [point_list?.total4 || 16, rows[0].phone]
        );
        await connection.query(
    ...
    if (data == 5) {
      ...
      if (check >= get && point_list?.total5 != 0) {
        await connection.query(
          "UPDATE users SET money = money + ? WHERE phone = ? ",
          [point_list?.total5 || 24, rows[0].phone]
        );
        await connection.query(
    ...
    if (data == 6) {
      ...
      if (check >= get && point_list?.total6 != 0) {
        await connection.query(
          "UPDATE users SET money = money + ? WHERE phone = ? ",
          [point_list?.total6 || 28, rows[0].phone]
        );
        await connection.query(
    ...
    if (data == 7) {
      ...
      if (check >= get && point_list?.total7 != 0) {
        await connection.query(
          "UPDATE users SET money = money + ? WHERE phone = ? ",
          [point_list?.total7 || 30, rows[0].phone]
        );
        await connection.query(
...
const promotion = async (req, res) => {
  ...
  const [f1_today] = await connection.query(
    "SELECT `phone`, `code`,`invite`, `time` FROM users WHERE `referredBy_ID` = ? AND time >= NOW() - INTERVAL 24 HOUR",
    [userInfo.id]
  ); // Referral code f1
  const [recharge_total] = await connection.query(
    "SELECT u.phone, SUM(r.money) AS total FROM users u LEFT JOIN recharge r ON u.phone = r.phone WHERE u.referredBy_ID = ? GROUP BY u.phone",
    [userInfo.id]
  );

  const [withdraw_total] = await connection.query(
    "SELECT u.phone, SUM(COALESCE(r_k3.money, 0) + COALESCE(r_5d.money, 0) + COALESCE(m1.money, 0)) AS total FROM users u LEFT JOIN result_k3 r_k3 ON u.phone = r_k3.phone LEFT JOIN result_5d r_5d ON u.phone = r_5d.phone LEFT JOIN minutes_1 m1 ON u.phone = m1.phone WHERE u.referredBy_ID = ? GROUP BY u.phone",
    [userInfo.id]
  );
  const totalbonus =
    (1 / 1000) *
  ...
  try {
      ...
      upload.single('file')(req, res, async (err) => {
          if (err instanceof multer.MulterError) {
            ...
            console.log(auth);
            console.log(token);
              // Verify that auth and token exist
              if (!auth || !token) {
                return res.status(401).json({ message: 'Authentication failed. Missing auth or token headers.' });
            }

            const [, authToken] = auth.split('=');
            // Query to retrieve user information based on token
            ...
            // Assuming the token uniquely identifies a user, use the first row
            const user = userRows[0];
            console.log(user);


          // No errors, continue processing the request
          const screenshotData = req.body.screenshot;

          // Save the screenshot with Multer
          const fileUrl = await saveScreenshotWithMulter(screenshotData);

          if (!fileUrl) {
              return res.status(400).json({ message: 'Error saving screenshot', status: false });
          }
          console.log('IIIIIIIIIIIIIIIIIIIIIII');
          console.log(req.user);
          // console.log(req.body);
          const userId = user ? user.id || 1 : 1; // Assuming user ID is available in req.user
          const mobile = user ? user.phone || null : null; // Set to null if undefined
          const clientId = user ? user.name_user || null : null; // Set to null if undefined
          const amount = req.body.money || null; // Set to null if undefined
          const utr = req.body.utr || null; // Set to null if undefined
          const requestDate = new Date().toISOString().split('T')[0];
          const type = req.body.type || null; // Set to null if undefined

          // Insert a new entry into the fund_requests table
          const sql = `INSERT INTO fund_requests 
                      (user_id, mobile, client_id, image, state, amount, status, request_date, utr, typeID) 
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
          await connection.execute(sql, [
              ...
              clientId,
              fileUrl,
              'pending', // Set state to null initially
              amount,
              'pending', // Set status to 'pending' by default
              requestDate,
              utr, // Include UTR number in the INSERT query
              type
          ]);
...
//             // Gather other parameters from the request body
//             const userId = "req.user.id; "// Assuming user ID is available in req.user
//             const mobile = "req.body.mobile;"
//             const clientId = "req.body.clientId;"
//             const amount = req.body.amount;
//             const utr = req.body.utr; // Extract UTR number from request body
...
//             // Insert a new entry into the fund_requests table
//             const sql = `INSERT INTO fund_requests 
//                         (user_id, mobile, client_id, image, state, amount, status, request_date, utr) 
//                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
//             await connection.execute(sql, [
//                 userId,
//                 mobile,
//                 clientId,
//                 fileUrl,
//                 null, // Set state to null initially
//                 amount,
//                 'pending', // Set status to 'pending' by default
//                 requestDate,
//                 utr // Include UTR number in the INSERT query
...
const infoUserBank = async (req, res) => {
  ...
  const [user] = await connection.query(
    "SELECT `phone`, `code`,`invite`, `money` ,`bonusMoney`FROM users WHERE `token` = ?",
    [auth]
  );
  let userInfo = user[0];
  if (!user) {
...
const withdrawal3 = async (req, res) => {
  ...
  const [user] = await connection.query(
    "SELECT `phone`, `code`,`invite`, `money`,`bonusMoney` FROM users WHERE `token` = ?AND password = ?",
    [auth, md5(password)]
  );
  ...
  if (user_bank.length != 0) {
    if (withdraw.length < 2) {
      if (userInfo.bonusMoney - money >= 0) {
        if (result >= 0) {
          ...
          await connection.query(
            "UPDATE users SET bonusMoney = bonusMoney - ? WHERE phone = ? ",
            [money, userInfo.phone]
          );
          return res.status(200).json({
            message: "Withdrawal successful",
            status: true,
            money: userInfo.bonusMoney - money,
            timeStamp: timeNow,
          });
        } else {
          return res.status(200).json({
...
const listRecharge = async (req, res) => {
  ...
  let userInfo = user[0];
  const rechargeHistory = await prisma.aviatorrecharge.findMany({
    where: {
...
const useRedenvelope = async (req, res) => {
  ...
  if (redenvelopes.length == 0) {
    ...
    if (infoRe.status == 0) {
      ...
      // Check if giftType is 'winning balance' or 'wallet balance'
      if (infoRe.gift_type === 'winning balance') {
        await connection.query(
          "UPDATE users SET bonusMoney = bonusMoney + ? WHERE `phone` = ? ",
          [infoRe.money, userInfo.phone]
        );
      } else if (infoRe.gift_type === 'wallet balance') {
        await connection.query(
          "UPDATE users SET money = money + ? WHERE `phone` = ? ",
          [infoRe.money, userInfo.phone]
        );
