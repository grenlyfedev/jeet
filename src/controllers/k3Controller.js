...
import {
  ...
  calculateTotalMoneyPlaced,
  fillMissingNumbers,
  findLowestPotentialPayout,
  ...
  if (money >= 10000) {
    if (f1.length > 0) {
      ...
      await connection.query(
        "UPDATE users SET bonusMoney = bonusMoney + ?, roses_f1 = roses_f1 + ?, roses_f = roses_f + ?, roses_today = roses_today + ? WHERE phone = ? ",
      ...
      if (f2.length > 0) {
        ...
        await connection.query(
          "UPDATE users SET bonusMoney = bonusMoney + ?, roses_f = roses_f + ?, roses_today = roses_today + ? WHERE phone = ? ",
...
const betK3 = async (req, res) => {
  try {
    ...
    const [user] = await connection.query(
      "SELECT `phone`, `code`, `invite`, `level`, `money`,`bonusMoney` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ",
      [auth]
    );
    ...
    if (userInfo.money - total >= 0) {
      ...
      check = userInfo.bonusMoney - total;
      tp = 1;
    }
    if (check >= 0) {
      ...
      const result_k31 = await connection.execute(sql, [
        id_product,
        userInfo.phone,
        userInfo.code,
        userInfo.invite,
      ...
      if (tp === 0) {
        ...
        await connection.execute(
          "UPDATE `users` SET `bonusMoney` = `bonusMoney` - ? WHERE `token` = ? ",
          [total, auth]
        );
      }
...
async function makeid(period) {
  ...
  const sql = `SELECT * FROM result_k3`;
  const [allBets1] = await connection.execute(sql);
  const allBets = allBets1.filter((item) => item.stage == `${period}`);
  if (allBets.length === 0) {
    const value = generateRandomThreeDigitNumber(3);
    return value;
  }
  const totalMoneyPlaced = calculateTotalMoneyPlaced(allBets);
  const potentialWinnings = calculatePotentialWinnings(
    betObject,
    totalMoneyPlaced
  );
  const potentialWinnings1 = fillMissingNumbers(potentialWinnings);
  const lowestPotentialPayoutBet1 =
    findLowestPotentialPayout(potentialWinnings1);
  let result = generateThreeDigitNumber(lowestPotentialPayoutBet1);
  return result;
}
const addK3 = async (game) => {
  try {
    ...
    let result2 = await makeid(period);
